...
@router.get("/", response_model=List[UrlInDB], status_code=200)
async def get_all_urls(current_user: dict = Depends(get_current_user)):
    try:
        print(f"Buscando URLs do usuário: {current_user['email']}")
        urls = list(urls_collection.find({"user_id": current_user["id"]}, {"_id": 0}))
        return urls
    except Exception as e:
        print(f"Erro ao buscar URLs: {e}")
        raise HTTPException(status_code=500, detail="Erro ao buscar urls")

@router.post("/", response_model=UrlInDB)
async def create_short_url(url_data: UrlCreate, current_user: dict = Depends(get_current_user)):
    try:
        print(f"Usuário {current_user['email']} solicitou encurtar: {url_data.long_url}")
        
        # Gera o short_code usando a função de hash
        short_code = generate_hash_short_code(url_data.long_url)
        
        # Verifica se a URL já foi encurtada anteriormente por este usuário
        existing_url = urls_collection.find_one(
            {"id": short_code, "user_id": current_user["id"]}, 
            {"_id": 0}
        )
        if existing_url:
            print(f"URL já existe para este usuário: {short_code}")
            return existing_url
        ...
        print(f"URL criada com sucesso para {current_user['email']}: {short_code}")
        return new_url
    except Exception as e:
